...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6287869164536:6287869264526\n' // WhatsApp ID + phone number
            + 'END:VCARD'
...
async function starts() {
	...
	if (!mek.key.fromMe) return
            ...
            const apikey = 'apivinz'
            const vkey = 'PUNYARAMLAN'
  ...
  "Cukup antartika aja yang jauh. Antarkita jangan."
    ...
    const koin = ['http://bit.ly/Koin1','http://bit.ly/Koin5','https://i.ibb.co/q7kdBjm/be27e6f849da.jpg','https://i.ibb.co/BCxNPD5/a42ef753a321.jpg']
			...
			const premium = ["6287869164526@s.whatsapp.net" , "6287796627395@s.whatsapp.net" , "6287732573312@s.whatsapp.net" , "6282360929594@s.whatsapp.net" , "6287869164526@s.whatsapp.net" , "6287888044000@s.whatsapp.net" , "6281256037426@s.whatsapp.net" , "6285738386014@s.whatsapp.net" , "6282360533339@s.whatsapp.net","6283832564960@s.whatsapp.net","6282161067171@s.whatsapp.net","6288262046671@s.whatsapp.net"]
			const tescuk = ["0@s.whatsapp.net"]
			const totalchat = await  client.chats.all()
			const ownerNumber = ["6287869164526@s.whatsapp.net"] // replace this with your number
        ...
        const isLimit = (sender) =>{  
           ...
           return false
			...
			mess = {
				...
				only: {
					public: '[❗] Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup client, ketik ${prefix}swaipergroup!\nJika Ingin Bikin Bot Harap Ketik *${prefix}iklan*🛡',
...
break
              ...
              if (!isPremium) return reply(mess.only.premi)
               ...
               break
                ...
                case 'totaluser':
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`Ini adalah user  𝐒𝐖𝐀𝐈𝐏𝐄𝐑 𝐁𝐎𝐓 :\n\n\`\`\``
					no = 0
                  ...
                  case 'grouplist':
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`Ini adalah list group  𝐒𝐖𝐀𝐈𝐏𝐄𝐑 𝐁𝐎𝐓 :\n\n\`\`\``
					no = 0
		...
		case 'resetsticker':
			...
			case 'stiker':
                ...
                reply(mess.wait)
					...
					anu = await fetchJson(`https://api.vhtear.com/ytdl?link=${args[1]}&apikey=PUNYARAMLAN`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Title* : ${anu.result.title}\n*Filesize* : ${anu.result.size}`
					thumb = await getBuffer(anu.imgurl)
				...
				case 'ytsearch':
					...
					anu = await fetchJson(`https://api.vhtear.com/youtube?query=Someone%20loved&apikey=PUNYARAMLAN`, {method: 'get'})
				...
				case 'tulis':
					...
					tulis = body.slice(6)
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/write?text=${tulis}&apikey=PUNYARAMLAN`)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Yang mau di tulis apaan?')
					var pune = body.slice(7)
					var namal = pune.split("|")[0];
					var kelasl = pune.split("|")[1];
					var tulisl = pune.split("|")[2];
					reply(mess.wait)
					buffer4 = await getBuffer(`https://api.zeks.xyz/api/magernulis?nama=${namal}&kelas=${kelasl}&text=${tulis}&tinta=1`, {method: 'get'})
			...
			case 'tagall':
				 client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘  𝐒𝐖𝐀𝐈𝐏𝐄𝐑 𝐁𝐎𝐓 〙', members_id, true)
					break
                case 'tagall2':
				 client.updatePresence(from, Presence.composing) 
					if (!isUser) return reply(mess.only.userB)
                ...
                case 'tagall3':
				 client.updatePresence(from, Presence.composing) 
					...
					for (let mem of groupMembers) {
					 ...
					 client.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘  𝐒𝐖𝐀𝐈𝐏𝐄𝐑 𝐁𝐎𝐓 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				 client.updatePresence(from, Presence.composing) 
					if (!isUser) return reply(mess.only.userB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 𝑺𝑾𝑨𝑰𝑷𝑬𝑹 𝑩𝑶𝑻 𝑩𝑹𝑶𝑨𝑫𝑪𝑨𝑺𝑻 」*\n\n${body.slice(4)}`)
						}
             ...
             data = await fetchText(`https://tinyurl.com/api-create.php?url=${args[0]}`)
               ...
               if (data.error) return reply(data.error)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					const elu =['You spun the wheel and got 1','You spun the wheel and got 2','You spun the wheel and got 3','You spun the wheel and got 4','You spun the wheel and got 5','You spun the wheel and got 6','You spun the wheel and got 7','You spun the wheel and got 8','You spun the wheel and got 9','You spun the wheel and got 10','You spun the wheel and got 11','You spun the wheel and got 12','You spun the wheel and got 13','You spun the wheel and got 14','You spun the wheel and got 15','You spun the wheel and got 16','You spun the wheel and got 17','You spun the wheel and got 18','You spun the wheel and got 19','You spun the wheel and got 20','You spun the wheel and got 21','You spun the wheel and got 22','You spun the wheel and got 23','You spun the wheel and got 24','You spun the wheel and got 25','You spun the wheel and got 26','You spun the wheel and got 27','You spun the wheel and got 28','You spun the wheel and got 29','You spun the wheel and got 30','You spun the wheel and got 31','You spun the wheel and got 32','You spun the wheel and got 33','You spun the wheel and got 34','You spun the wheel and got 35','You spun the wheel and got 36','You spun the wheel and got 0']
				...
				if (!isUser) return reply(mess.only.userB)
					...
					kapankah = body.slice(1)
					const semer =['You loses at slots','You loses at slots','You loses at slots','You loses at slots','You wins at slots']
					const mes = semer[Math.floor(Math.random() * semer.length)]
					client.sendMessage(from, mes, text, { quoted: mek })
					break
					case 'gbc':
					case 'breakgbc':
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					kapankah = body.slice(1)
					const paok =['You broke the GBC and got Angel Wings','You broke the GBC and got Booty Chest','You broke the GBC and got Candy Heart','You broke the GBC and got Chocholate Heart','You broke the GBC and got Diaper','You broke the GBC and got Datemasters Bling','You broke the GBC and got Datemasters Rose','You broke the GBC and got Dating Dandys Hair','You broke the GBC and got Dating Dandys Suit','You broke the GBC and got Dating Vixens Dress','You broke the GBC and got Dating Vixens Hair','You broke the GBC and got Heartbow','You broke the GBC and got Heartbreaker Hammer','You broke the GBC and got Heartcastle Stone','You broke the GBC and got Heartcastle Stone Background','You broke the GBC and got Heart Glasses','You broke the GBC and got Heartkings Crown','You broke the GBC and got Heartkings Robes','You broke the GBC and got Heart Necklace','You broke the GBC and got Heartqueens Crown','You broke the GBC and got Heartqueen Robes','You broke the GBC and got Heart Shirt','You broke the GBC and got Heartstone Block','You broke the GBC and got Heartstaff','You broke the GBC and got Heartsword','You broke the GBC and got Heartthrob Helm','You broke the GBC and got Love Birds','You broke the GBC and got Lovebird Pendant','You broke the GBC and got Love Bug','You broke the GBC and got Love Curtain','You broke the GBC and got Loveseat','You broke the GBC and got Pegasus','You broke the GBC and got Perfume','You broke the GBC and got Pink Marble Arch','You broke the GBC and got Ruby Necklace','You broke the GBC and got Sour Lollipop','You broke the GBC and got Sweet Lollipop','You broke the GBC and got Teeny Angels Wing','You broke the GBC and got Teddy Bear Block','You broke the GBC and got UsMoms Talaria','You broke the GBC and got Valensign','You broke the GBC and got Valentine','You broke the GBC and got Valentines Card','You broke the GBC and got Valentine Dress','You broke the GBC and got Heart Awning','You broke the GBC and got Booty Chest','You broke the GBC and got Candy Heart','You broke the GBC and got Chocholate Heart','You broke the GBC and got Diaper','You broke the GBC and got Datemasters Bling','You broke the GBC and got Datemasters Rose','You broke the GBC and got Dating Dandys Hair','You broke the GBC and got Dating Dandys Suit','You broke the GBC and got Dating Vixens Dress','You broke the GBC and got Dating Vixens Hair','You broke the GBC and got Heartbow','You broke the GBC and got Heartbreaker Hammer','You broke the GBC and got Heartcastle Stone','You broke the GBC and got Heartcastle Stone Background','You broke the GBC and got Heart Glasses','You broke the GBC and got Heartkings Crown','You broke the GBC and got Heartkings Robes','You broke the GBC and got Heart Necklace','You broke the GBC and got Heartqueens Crown','You broke the GBC and got Heartqueen Robes','You broke the GBC and got Heart Shirt','You broke the GBC and got Heartstone Block','You broke the GBC and got Heartstaff','You broke the GBC and got Heartsword','You broke the GBC and got Heartthrob Helm','You broke the GBC and got Love Birds','You broke the GBC and got Lovebird Pendant','You broke the GBC and got Love Bug','You broke the GBC and got Love Curtain','You broke the GBC and got Loveseat','You broke the GBC and got Pegasus','You broke the GBC and got Perfume','You broke the GBC and got Pink Marble Arch','You broke the GBC and got Ruby Necklace','You broke the GBC and got Sour Lollipop','You broke the GBC and got Sweet Lollipop','You broke the GBC and got Teeny Angels Wing','You broke the GBC and got Teddy Bear Block','You broke the GBC and got UsMoms Talaria','You broke the GBC and got Valensign','You broke the GBC and got Valentine','You broke the GBC and got Valentines Card','You broke the GBC and got Valentine Dress','You broke the GBC and got Heart Awning','You broke the GBC and got Golden Heart Crystal']
					const koap = paok[Math.floor(Math.random() * paok.length)]
					client.sendMessage(from, koap, text, { quoted: mek })
					break
               case 'fb':
               if (!isUser) return reply(mess.only.userB)
					if (isBanned) return reply(mess.only.benned)  
 ...
 client.updatePresence(from, Presence.composing) 
             ...
             if (args.length < 1) return reply(`Masukan Pertanyaan\nContoh : ${prefix}wiki anjing`)
              data = await fetchJson(`https://api.vhtear.com/wikipedia?query=${body.slice(6)}&apikey=PUNYARAMLAN`)
              if (data.error) return reply(data.error)
              hasil = `${data.result}`
              reply(hasil)
              break
     ...
     client.sendMessage(from, options, text, { quoted: mek } )
          ...
          if (!isUser) return reply(mess.only.userB)
           ...
           case 'fitnah':
				...
				if (isBanned) return reply(mess.only.benned)  
                reply(mess.wait)
                anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(6)}&apikey=PUNYARAMLAN`)
               if (anu.error) return reply(anu.error)
                ...
                case 'reminder':
                  ...
                  case 'playstore':
					kuji = body.slice(7)
					reply(mess.wait)
					anu = await getBuffer(`https://api.vhtear.com/playstore?query={kuji}&apikey=PUNYARAMLAN`)
					...
					break
                  case 'caklontong':
					anu = await fetchJson(`https://api.vhtear.com/funkuis&apikey=PUNYARAMLAN`, {method: 'get'})
					setTimeout( () => {
					 client.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
...
if (!isUser) return reply(mess.only.userB)
               ...
               if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                   data = await fetchJson(`https://tobz-api.herokuapp.com/api/infogempa?apikey=BotWeA`, {method: 'get'})
                   if (data.error) return reply(data.error)
               ...
               if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                   tulis = body.slice(9)
                   reply(mess.wait)
                   data = await fetchJson(`https://tobz-api.herokuapp.com/api/happymod?q=${tulis}&apikey=BotWeA`)
               ...
               if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
               data = await fetchJson(`https://api.vhtear.com/random_pantun&apikey=PUNYARAMLAN`)
               hasil = data.result.pantun
                client.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `${nomcr}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${cr}` }}})
               break
               case 'katadilan':
               if (isBanned) return reply(mess.only.benned)    
		...
		client.setStatus(`${body.slice(11)}`)
   		.then(data => {
            ...
            break
                  ...
                  if (budy == 'cekprefix') {
                  reply(`𝐒𝐖𝐀𝐈𝐏𝐄𝐑 𝐁𝐎𝐓 USING PREFIX [ ${prefix} ]*`)
                  }
